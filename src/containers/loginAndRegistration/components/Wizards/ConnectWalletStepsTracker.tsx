...
type LoginWizardStepStatus = 'completed' | 'current' | 'in-processing' | 'pending'
...
export const ConnectWalletStepsTracker = ({ steps }: ConnectWalletStepsTrackerProps) => {
  ...
  const calcStepBaseStepDescClass = (status: LoginWizardStepStatus) => {
    return classNames(status === 'pending' ? 'text-inactive-300' : 'text-secondary-950')
  }
  const getStepIndexContent = (stepIndex: number, status: LoginWizardStepStatus) => {
    if (status === 'completed') {
      ...
      {steps.map((step, index) => (
        <Fragment key={index}>
          <div className="flex gap-4">
            <div className={calcStepIconClass(step.status)}>{getStepIndexContent(index, step.status)}</div>
            <div className={calcStepBaseStepDescClass(step.status)}>{step.description}</div>
          </div>
