...
import { useConnectToNetworkWizardLogic } from '../loginAndRegistration/logic/useConnectToNetworkWizardLogic'
import { ConnectToNetworkWizard } from '../loginAndRegistration/ConnectToNetworkWizard'

export const UserInfoHeader = () => {
  const navigator = useNavigator()
  const { terminateSession } = useSession()
  const { address, isConnectedToProperWallet } = useCurrentWalletInfo()
  const { showConnectToNetworkWizard } = useConnectToNetworkWizardLogic()
        ...
        <div className="basis-2/12 md:order-2 py-4 flex justify-center md:justify-end">
          <div>
            <div className="flex gap-2">
              {isConnectedToProperWallet && address ? (
                ...
                <Button variant="primary" layout="compact" onClick={showConnectToNetworkWizard}>
                  Connect
                </Button>
              )}
              <div>
                <Button variant="primary" layout="icon-only" onClick={handleLogoutClick} aria-label="Logout">
                  <div className="w-4 h-4 flex justify-center items-center">
                    <FontAwesomeIcon icon={faArrowRightFromBracket} className="w-4 h-4 text-primary-950" />
                  </div>
                </Button>
              </div>
            </div>
