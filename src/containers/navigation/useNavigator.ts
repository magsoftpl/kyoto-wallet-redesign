...
type LinkFn = <T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => string
type NavigationFn = <T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => void
...
export const useLinkProvider = (): LinkFn => {
  return useCallback(
    <T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => (urlMapping[target] as any)(...args),
    [],
  )
}

export const useRedirector = (): NavigationFn => {
  return useCallback(<T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => {
...
export const useNavigator = (): GoToNavigationFn => {
  ...
  const result = useMemo(() => {
    const goToFn = (<T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => {
      const targetUrl = (urlMapping[target] as any)(...args)
      logInfo('Navigate', targetUrl)
      router.push(targetUrl)
    }) as GoToNavigationFn
    const replaceFn = <T extends OnrampUrl>(target: T, ...args: Parameters<UrlMapping[T]>) => {
      const targetUrl = (urlMapping[target] as any)(...args)
