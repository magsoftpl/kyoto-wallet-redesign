...
export interface TransactionBlockChainFailedProps {
  chain: WalletChain
  txHash: string
}

export const TransactionBlockChainFailed = ({ chain, txHash }: TransactionBlockChainFailedProps) => {
  return (
    <div className="w-full h-full p-4 pb-8 flex flex-col gap-8 justify-center uppercase text-center">
      <div className="w-full px-12">
        <div className="h-16 p-2 rounded-full border-[6px] border-solid border-red">
          <div className="h-full p-1 w-1/3 rounded-l-full bg-red" />
        </div>
      </div>
      <div className="min-h-[3.2em] text-red">Your transaction failed</div>
      ...
      <div className="w-full px-8">
        <Pill variant="error">Confirmation complete</Pill>
      </div>
    </div>
  )
}

export const transactionBlockChainFailedTitle = 'Transaction failed'
