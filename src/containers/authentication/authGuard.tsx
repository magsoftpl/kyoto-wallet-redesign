...
export const AuthGuard = ({
  ...
  useEffect(() => {
    const isRedirect =
      redirectTarget && !allowContent(sessionRestriction, session);
    if (isRedirect) {
...
function allowContent(
  sessionRestriction: SessionRestriction | undefined,
  session: unknown
) {
  return (
    (sessionRestriction === "required" && !!session) ||
    (sessionRestriction === "prohibited" && !session)
  );
}
